...
| `make init-sql` | 按顺序执行 `migrations/*.sql` 初始化数据库 |
...
- 如需沉淀未实现设计，可新增 `docs/plan/`；未开工或未落地的方案不要写进 `docs/product/`。
...
| 目录 | 回答的问题 | 时态 |
| --- | --- | --- |
| `docs/product/` | 系统对用户、运营或管理员呈现出的产品能力和业务规则 | 当前已实现 |
| `docs/architecture/` | 系统怎么组织、服务边界在哪里、运行和通信方式是什么 | 长期架构 |
| `docs/api/` | HTTP/WebSocket 接口怎么调用，请求响应是什么 | 当前已实现 |
| `docs/dev-phase/` | 做了什么、做到哪、如何验证、还差什么 | 进行中 / 已完成 |
| `docs/plan/` | 将来要做什么、怎么设计、还有哪些 TODO | 未来未实现 |

产品文档编写原则：

- `docs/product/` 永远描述“当前系统长什么样”，只写已经实现或确定落地的产品能力。
- 未开工、未确认或仅是想法的设计先写入 `docs/plan/`；开工后在 `docs/dev-phase/` 跟进，完成后再沉淀回 `docs/product/`。
- 产品文档从用户、运营、租户、管理员等角色视角描述能力、规则、边界和当前状态，不展开代码调用链、类名、函数名和 SQL 实现细节。
- 每篇 product 文档应聚焦一个业务域，优先包含：产品目标、核心对象、关键规则、权限/隔离边界、后台运营能力、当前状态。
- 涉及余额、积分、Token、资产、权限、审计等关键业务，必须写清归属关系、可见范围、是否可追溯和异常限制。
- 如果 API 行为改变，`docs/api/` 和对应 `docs/product/` 都要同步；如果只改内部实现但产品能力不变，通常只更新 `docs/dev-phase/` 或架构文档。
...
- 面向 Swagger / Scalar / ReDoc 展示的文本优先使用中文，方便开发和业务查看。
- 路由必须尽量补充 `summary`，复杂接口补充 `description`；文案描述业务动作，不写实现细节。
- `APIRouter(tags=[...])`、路径参数 `Path(description=...)`、查询参数 `Query(description=...)`、请求体字段 `Field(description=...)` 都应使用清晰中文。
- `app/schemas/` 中的请求/响应模型字段应补中文 `description`；枚举字段要说明可选值的业务含义。
- Schema 类名、字段名和 enum 成员名保持英文代码风格；只有 OpenAPI 展示文案、API summary、description、字段说明使用中文。
- 新增或修改接口后，应检查 `/openapi.json` 或 Scalar 页面，确认核心请求体、响应体、参数说明可读。

## 枚举、错误码和异常分类

固定协议值、业务状态、权限名、错误码和对外错误消息必须使用 `StrEnum` 或集中常量，不要在业务流程中散落裸字符串。

推荐放置位置：

| 类型 | 推荐位置 | 说明 |
| --- | --- | --- |
| 跨领域错误码 | `app/core/error_codes.py` | 全局稳定错误码，例如 `AUTH_INVALID_CREDENTIALS` |
| 跨领域错误消息 | `app/core/error_messages.py` | 面向用户/API 调用方的稳定消息 |
| 单领域枚举 | `app/services/<domain>/enums.py` | 领域内状态、类型、动作，例如登录方式、API Key 状态 |
| 单领域异常 | `app/services/<domain>/exceptions.py` | 领域语义异常，例如 `InvalidCredentialsException` |
| 基础异常与 handler | `app/core/exceptions.py` | `APIException` 基类和全局异常处理器 |

错误码和错误消息应稳定、可测试、可文档化。错误消息可参考以下模式：
...
class ErrorMessage(StrEnum):
    """Stable backend error messages for user-facing responses."""

    INVALID_CREDENTIALS = "Invalid username or password."
    API_KEY_EXPIRED = "API key has expired."
    CANNOT_DISABLE_SELF = "You cannot disable your own admin account."
    UNKNOWN_PERMISSION = "Unknown permission: {permission}"
```

异常分类建议：

- `app.core.exceptions` 只定义通用 HTTP/API 语义，例如 `BadRequestException`、`UnauthorizedException`、`ForbiddenException`、`NotFoundException`、`ConflictException`。
- 业务 service 不应抛裸 `ValueError` 表示可预期业务失败；应抛领域异常或通用 `APIException` 子类。
- API 路由不应通过字符串判断异常内容来决定 HTTP 状态码；状态码、错误码和错误消息应由异常类型决定。
- Repository 捕获数据库唯一约束、外键约束等底层异常时，应映射为稳定业务异常，不要把驱动错误文本直接传给 API 调用方。
- 对外错误消息必须避免泄露密码、JWT、API secret、TOTP secret、私钥、数据库连接信息、SQL 语句、第三方 token 等敏感内容。
- 内部日志可以记录更完整的错误链，但日志字段必须稳定，例如 `error_code`、`user_id`、`resource_id`、`operation`。
...
class InvalidCredentialsException(UnauthorizedException):
    def __init__(self) -> None:
        super().__init__(message=ErrorMessage.INVALID_CREDENTIALS)
```
...
- 错误处理优先使用 `app.core.exceptions` 和领域异常；不要在业务层随意返回裸 dict、抛裸 `ValueError` 或在 API 层用字符串匹配异常消息。
